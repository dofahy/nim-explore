...
type Section* = ref object
  properties: OrderedTable[string, string]
...
proc newSection*(): Section =
  var s = Section()
  s.properties = initOrderedTable[string, string]()
...
type Ini* = ref object
  sections: OrderedTable[string, Section]

proc newIni*(): Ini =
  var ini = Ini()
  ini.sections = initOrderedTable[string, Section]()
  return ini
...
proc hasProperty*(this: Ini, sectionName: string, key: string): bool =
  return
    this.sections.contains(sectionName) and
    this.sections[sectionName].properties.contains(key)
...
proc deleteProperty*(this: Ini, sectionName: string, key: string) =
  if this.sections.contains(sectionName) and
      this.sections[sectionName].properties.contains(key):
...
proc normaliseLines(s: string): seq[string] =
  let raw = s.splitLines()
  var nline: seq[string] = @[]
  var buffer = ""

  for line in raw:
    let stripped = line.strip(trailing = true, leading = false)

    if stripped.endsWith("\\"):
      buffer &= stripped[0 ..< stripped.len - 1] & " "
    else:
      nline.add(buffer & stripped)
      buffer = ""
  ...
  for rawLine in lines:
    ...
    if line.startsWith("["):
      if line.endsWith("]"):
        ...
        raise newException(
          ValueError, fmt("Excpected line {line} to start with [ and end with ]")
        )
